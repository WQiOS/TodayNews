...
class RefreshHeder: MJRefreshGifHeader {
    ...
    override func prepare() {
        ...
        var images = [UIImage]()
        for index in 0..<30 {
            let image = UIImage.init(named: "dropdown_0\(index)")            
            images.append(image!)
        }
...
//        gifView.frame = CGRect(x: 0, y: 5, width: mj_w, height: 40)
//        stateLabel.font = UIFont.systemFont(ofSize: 12)
//        stateLabel.frame = CGRect(x: screenWidth/2 + 30, y: 10, width: mj_w, height: 29)
//        self.bringSubview(toFront: stateLabel)
    }
}
